...
const HyoBank = artifacts.require('HyoBank');

module.exports = function(deployer) {
	deployer.deploy(HyoBank).then(() => {
		const data = JSON.stringify({
			contractAddress: HyoBank.address
		});
		...
		console.log(`\n    Create file of contract address to json: ${HyoBank.address}`);

		const abi = JSON.stringify(HyoBank._json.abi);
		fs.writeFileSync('./artifacts/abi.json', abi);
		console.log(`\n    Create file of abi file to json: ${HyoBank.address}`);
		...
		uploader.uploadArtifacts(HyoBank._json.contractName, 'artifacts', containerName);
	});
};
